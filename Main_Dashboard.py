...
# --- PAGE 2: HR ONBOARDING (THE FOOLPROOF STEP) ---
    ...
    st.write("Register a new employee in the Registry and Rekognition simultaneously.")
    st.markdown("---")
            ...
            try:
                ...
                with st.spinner("Processing Onboarding..."):
                    ...
                    try:
                        ...
                        st.info("Step 1: Record added to DynamoDB Registry.")
                    except Exception as db_e:
                        st.error(f"❌ DynamoDB Error: {str(db_e)}")
                        raise db_e # Stop if DB fails
                    ...
                    try:
                        rekognition.index_faces(
                            ...
                            ExternalImageId=new_emp_id,
                            DetectionAttributes=['ALL']
                        )
                        st.info("Step 2: Biometrics indexed in Rekognition.")
                    except Exception as rek_e:
                        st.error(f"❌ Rekognition Error: {str(rek_e)}")
                        raise rek_e

                st.success(f"✅ Employee {new_emp_id} fully onboarded!")
                st.balloons()
            except Exception as e:
                # Final fallback error message
                st.error("Onboarding halted due to the error above.")
        else:
            st.warning("Please fill all fields and capture a photo.")

st.sidebar.markdown("---")
st.sidebar.info("System is connected to ap-south-1 (Mumbai)")